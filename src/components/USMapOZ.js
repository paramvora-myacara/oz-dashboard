...
export default function USMapOZ() {
  ...
  const StateTooltip = ({ state }) => {
    ...
    return (
      <div className="bg-white/95 backdrop-blur-xl rounded-xl shadow-2xl p-6 min-w-[280px] border border-gray-200/50">
        <h3 className="text-xl font-bold text-gray-900 mb-4">{state}</h3>
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="text-gray-600">Opportunity Zones</span>
            <span className="font-semibold text-lg">{stats.totalZones}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-600">Total Investment</span>
            <span className="font-semibold text-green-600">{stats.investment}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-600">YoY Growth</span>
            <span className="font-semibold text-blue-600">{stats.growth}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-600">Avg Poverty Rate</span>
            <span className="font-semibold">{stats.avgPoverty}%</span>
          </div>
        </div>
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-2xl shadow-xl p-8 flex items-center justify-center h-[600px]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-4 border-blue-500 border-t-transparent mx-auto mb-4"></div>
          <p className="text-gray-600">Loading opportunity zones data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-6">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-xl">
            <h4 className="text-green-800 font-semibold">Active Zones</h4>
            <p className="text-2xl font-bold text-green-900">8,764</p>
            <p className="text-sm text-green-700">Across all states</p>
          </div>
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-xl">
            <h4 className="text-blue-800 font-semibold">Total Investment</h4>
            <p className="text-2xl font-bold text-blue-900">$105B+</p>
            <p className="text-sm text-blue-700">Since 2018</p>
          </div>
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-xl">
            <h4 className="text-purple-800 font-semibold">Jobs Created</h4>
            <p className="text-2xl font-bold text-purple-900">2.1M+</p>
            <p className="text-sm text-purple-700">Direct employment</p>
          </div>
        </div>
      </div>
    </section>
