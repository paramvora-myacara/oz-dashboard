    ...
    <div className="w-full h-full flex flex-col bg-white dark:bg-black">
      ...
      >
        ...
        {hoveredState && stateData && (
          <div 
            className="absolute glass-card rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 pointer-events-none z-50 animate-fadeIn bg-white/90 dark:bg-black/80 border border-black/10 dark:border-white/10"
            style={{
              left: `${Math.min(mousePosition.x + 15, dimensions.width - (dimensions.width < 640 ? 220 : 280))}px`,
              top: `${Math.min(mousePosition.y + 15, dimensions.height - (dimensions.width < 640 ? 140 : 180))}px`
            }}
          >
            <h3 className="text-lg sm:text-xl lg:text-2xl font-semibold text-black dark:text-white mb-2 sm:mb-3">{hoveredState}</h3>
            <div className="space-y-1 sm:space-y-2">
              <div className="flex justify-between gap-6 sm:gap-8 lg:gap-12">
                <span className="text-xs sm:text-sm text-black/60 dark:text-white/60">OZ Zones</span>
                <span className="text-xs sm:text-sm text-black dark:text-white font-medium">{stateData.zones}</span>
              </div>
              <div className="flex justify-between gap-6 sm:gap-8 lg:gap-12">
                <span className="text-xs sm:text-sm text-black/60 dark:text-white/60">Active Projects</span>
                <span className="text-xs sm:text-sm text-[#0071e3] font-medium">{stateData.activeProjects}</span>
              </div>
              <div className="flex justify-between gap-6 sm:gap-8 lg:gap-12">
                <span className="text-xs sm:text-sm text-black/60 dark:text-white/60">Investment Volume</span>
                <span className="text-xs sm:text-sm text-[#30d158] font-medium">${stateData.investmentBillions}B</span>
              </div>
            </div>
          </div>
        )}
